...
# % of missing values
load("data/acs.rda")
na <- ifelse(is.na(acs[1:25]), 1, 0) |> sum()
notNa <- ifelse(is.na(acs[1:25]), 0, 1) |> sum()
na/(na + notNa)
...
dfVarImp <- data.frame()
rownamesDfVarImp <- sort(rownames(varImpSvmMace$importance))

varImpSvmMace <- data.frame(varImpSvmMace$importance) 
varImpXgbMace <- data.frame(varImpXgbMace$importance) 
varImpSvmBleeding <- data.frame(varImpSvmBleeding$importance) 
varImpXgbBleeding <- data.frame(varImpXgbBleeding$importance) 

varImpSvmMace <- varImpSvmMace[order(rownames(varImpSvmMace)), ]
varImpXgbMace <- varImpXgbMace[order(rownames(varImpXgbMace)), ]
varImpSvmBleeding <- varImpSvmBleeding[order(rownames(varImpSvmBleeding)), ]
varImpXgbBleeding <- varImpXgbBleeding[order(rownames(varImpXgbBleeding)), ]

dfVarImp <- data.frame() |> 
  summarise(label = rownamesDfVarImp,
            scaledSvmMace = varImpSvmMace / max(varImpSvmMace),
            scaledXgbMace = varImpXgbMace / max(varImpXgbMace),
            scaledSvmBleeding = varImpSvmBleeding / max(varImpSvmBleeding),
            scaledXgbBleeding = varImpXgbBleeding / max(varImpXgbBleeding),
...
lrmMaceTrain <- predict(lrmMace)
svmMaceTrain <- predict(svmMace,
        newdata = complete(impTrain, 1),
        type="prob")[[2]]
xgbMaceTrain <- predict(xgbMace,
        newdata = complete(impTrain, 1),
        type="prob")[[2]]
lrmBleedingTrain <- predict(lrmBleeding)
svmBleedingTrain <- predict(svmBleeding,
        newdata = complete(impTrain, 1),
        type="prob")[[2]]
xgbBleedingTrain <- predict(xgbBleeding,
        newdata = complete(impTrain, 1),
        type="prob")[[2]]

roc(complete(impTrain, 1)$mace, lrmMaceTrain)
ci.auc(complete(impTrain, 1)$mace, lrmMaceTrain)
roc(complete(impTrain, 1)$mace, svmMaceTrain)
ci.auc(complete(impTrain, 1)$mace, svmMaceTrain)
roc(complete(impTrain, 1)$mace, xgbMaceTrain)
ci.auc(complete(impTrain, 1)$mace, xgbMaceTrain)
roc(complete(impTrain, 1)$majorBleeding, lrmBleedingTrain)
ci.auc(complete(impTrain, 1)$majorBleeding, lrmBleedingTrain)
roc(complete(impTrain, 1)$majorBleeding, svmBleedingTrain)
ci.auc(complete(impTrain, 1)$majorBleeding, svmBleedingTrain)
roc(complete(impTrain, 1)$majorBleeding, xgbBleedingTrain)
ci.auc(complete(impTrain, 1)$majorBleeding, xgbBleedingTrain)
...
rocMace <- ggroc(data = list(rocLrmMace,
  ...
  scale_color_manual(name = "",
                     labels = c("Regressão Estatística", "Support Vector Machine", "Extreme Gradient Boosting"),
                     values = c("#850A70", "#D61F28", "#156FA2")) +
...
# Calibration curve: mace
calMace <- ggplot(predictions,
       aes(y = mace)) +
  geom_abline(color = "darkgrey",
              linetype = "dashed") +
  geom_smooth(aes(x = lrmMace,
                  color = "Regressão Estatística"),
              se = FALSE) +
  geom_smooth(aes(x = svmMace,
                  color = "Support Vector Machine"),
              se = FALSE) +
  geom_smooth(aes(x = xgbMace,
                  color = "Extreme Gradient Boosting"),
              se = FALSE) +
  scale_color_manual(name = "",
                     breaks = c("Regressão Estatística",
...
# Calibration curve: bleeding
calBleeding <- ggplot(predictions,
       aes(y = bleeding)) +
  geom_abline(color = "darkgrey",
              linetype = "dashed") +
  geom_smooth(aes(x = lrmBleeding,
                  color = "Regressão Estatística"),
              se = FALSE) +
  geom_smooth(aes(x = svmBleeding,
                  color = "Support Vector Machine"),
              se = FALSE) +
  geom_smooth(aes(x = xgbBleeding,
                  color = "Extreme Gradient Boosting"),
              se = FALSE) +
  scale_color_manual(name = "",
                     breaks = c("Regressão Estatística",
...
crossRiskPlot <- ggplot(crossRisk,
       aes(y = n,
...
riskDiffMace <- ggplot(riskTradeoffMace,
       aes(x = centileMace,
           y = riskDiff,
  ...
  scale_color_manual(name = "",
                    labels = c("Alto risco hemorrágico",
                               "Intermediário risco hemorrágico",
                               "Baixo risco hemorrágico"),
                    values = c("#A1171E", "#D61F28", "#E64C54")) +
  guides(color=guide_legend(override.aes=list(fill=NA))) +
  labs(x = "\nPercentis de Risco Cardiovascular",
       y = "Diferença dos Riscos Cardiovascular e Hemorrágico (%)\n") +
...
riskDiffBleeding <- ggplot(riskTradeoffBleeding,
...
interventionsAvoidedMace <- dca(formula = mace ~ predictionLrmMace + predictionSvmMace + predictionXgbMace,
  ...
  plot(smooth = TRUE) +
  labs(x = "\nProbabilidade Limiar (%)\n",
       y = "\nIntervenções Evitadas a Cada 100 Pacientes\n") +
   scale_color_manual(labels = c("Regressão Estatística",
                                 "Support Vector Machine",
                                 "Extreme Gradient Boosting"), 
                      values = c("#850A70", "#D61F28", "#156FA2")) +
  scale_x_continuous(expand = expansion(0,0),
                     limits = c(0,0.205),
                     labels = scales::label_percent(suffix = "")) +
  scale_y_continuous(expand = expansion(0.025,0)) +
  theme_bw() +
  theme(panel.border = element_blank(),
        panel.background = element_blank(),
        panel.grid.major = element_blank(),
